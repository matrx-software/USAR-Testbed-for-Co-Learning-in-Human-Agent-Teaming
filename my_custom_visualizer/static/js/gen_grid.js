...
function draw(state, world_settings, new_messages, accessible_chatrooms, new_tick) {
    ...
    obj_ids.forEach(function(objID) {
        ...
        var obj_vis_settings = {
            ...
            "clickable": (obj.hasOwnProperty('clickable') && !obj['clickable'] ? true:false)
        };
        ...
        if (!saved_prev_obj_keys.includes(objID)) {
            ...
            // V2205 EDIT
            // set questionnaire link
            if (objID == 'setting') {
                let link = state[objID]['questionnaire_link']
                $("#endscreen a").attr("href", link)
            }
            ...
            // AIMS custom code, activate visual earthquake 
            if (objID == "vesuvius") {
                // console.log("Checking vesuvius", obj_element)
                if (obj['erupting_and_quaking']) {
                    // console.log("Earthquak happening")

                    let classList = Array.from($("#grid")[0].classList)
                    if (!classList.includes("shake")) {
                        $("#grid").addClass("shake")
                        // console.log("adding class")

                        // stop shaking after a couple of seconds 
                        setTimeout(function() {
                            $("#grid").removeClass("shake")
                        }, 1500);
                    }                   
                } 
            }
    ...
    // V2205 edit: show endscreen when goal reached 
    if (state['setting'].goal_reached) {
...
function fix_tile_size() {
    ...
    // AIMS specific, make sure there is space for the chat 
    available_width = 0.8 * available_width
